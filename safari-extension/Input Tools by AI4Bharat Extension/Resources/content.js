...
var yourTextarea = document.querySelector(".auto-resizable-textarea");
...
function handleInput($textObj) {
  ...
  // Trigger input event without causing recursion
  ...
  const requestData = {
    ...
    config: {
      ...
      language: {
        sourceLanguage: "en",
        sourceScriptCode: "",
        targetLanguage: languageCode, // Use the selected language
        targetScriptCode: "",
      },
      isSentence: false,
      numSuggestions: 5,
...
function dragSuggestionBox() {
  ...
  suggestionBox.onmousedown = function (e) {
    e.preventDefault();
  ...
  function elementDrag(e) {
    ...
    function updatePosition() {
      ...
      x = Math.min(
        Math.max(x, 0),
        window.innerWidth - suggestionBox.offsetWidth
      );
      y = Math.min(
        Math.max(y, 0),
